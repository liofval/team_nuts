...
リクエスト例:
```bash
curl "http://localhost:8080/api/v1/tags/suggest?q=IT&limit=10"
```
...
リクエスト例:
```bash
curl -X POST http://localhost:8080/api/v1/press_release/1/tags \
  -H "Content-Type: application/json" \
...
リクエスト例:
```bash
curl -X PUT http://localhost:8080/api/v1/press_release/1/tags/12 \
  -H "Content-Type: application/json" \
...
{
  "status": "ok",
  "tag": {"id": 12, "name": "情報技術", "slug": "joho-gijutsu", "type": "tag", "count": 453, "created_at": "2026-03-09T12:00:00Z"}
}
```
